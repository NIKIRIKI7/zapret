...
def parse_out_range(action_lines: list[str]) -> OutRangeSettings:
    for line in action_lines:
        ...
        if not stripped.startswith("--out-range="):
            continue
        value = line.split("=", 1)[1].strip()
        match = re.match(r"^([nd]?)(\d+)$", value.lower())
        if match:
            mode = match.group(1) or "n"
            return OutRangeSettings(enabled=True, value=int(match.group(2)), mode=mode, raw_line=line.strip())
...
def compose_action_lines(strategy_args: list[str], out_range: OutRangeSettings, send: SendSettings, syndata: SyndataSettings) -> list[str]:
    ...
    if out_range.enabled and out_range.value > 0:
        prefix = "" if out_range.mode == "n" else out_range.mode
        result.append(f"--out-range={prefix}{out_range.value}")
    if send.enabled:
