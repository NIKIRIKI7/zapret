...
def extract_category_from_args(args: str) -> Tuple[str, str, str]:
    ...
    # NOTE: some built-in lists use versioned ipset filename suffixes
    # (e.g. cloudflare-ipset_v6.txt). Treat those as the same logical
    # category key (cloudflare), otherwise round-trip through the preset
    # model creates a synthetic category like `cloudflare-ipset_v6` and
    # later writers may generate `ipset-cloudflare-ipset_v6.txt`.
    suffixes_to_remove = [
        '-hosts',
        '-ips',
        '-ipset_v6',
